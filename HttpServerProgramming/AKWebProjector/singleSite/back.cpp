...
int main()
{
    ...
    int i;
    char g;
    char responseBuffer[1025]; //we send a chunk of 1024 + 1 space for \0(string terminator)
    char requestBuffer[8193]; // 8192(8kb) 1024 x 8 + 1 space for \0(string terminator)
    int bytesExtracted;
    WORD ver;
    WSADATA wsaData;
    ...
    bytesExtracted=recv(clientSocketDescriptor,requestBuffer,8192,0); //in place of 8192 we can write "sizeof(request)-1"
    if(bytesExtracted<0)
    {
    ...
    else
    {
        ...
        if(request->isClientSideTechnologyResource)
        {
            if(request->resource==NULL)   
            {
                ...
                if(f==NULL)
                {
                    strcpy(responseBuffer,"HTTP/1.1 200 OK\nContent-Type:text/html\nContent-Length:163\n\n<!DOCTYPE HTML><html lang='en'><head><meta charset='utf-8'><title>AK Web Projector</title></head><body><h2 style='color:red'>Resource / not found </h2></body></html>");
                    //strcpy(responseBuffer,"HTTP/1.1 200 OK\nContent-Type:text/html\nContent-Length:163\n\n<!DOCTYPE HTML><html lang='en'><head><meta charset='utf-8'><title>AKWebProjector</title></head><body><h2 style='color:red'>Resource / not found</h2></body></html>");
            ...
            else
            {
                ...
                {
                    char tmp[501];
                    sprintf(tmp,"<!DOCTYPE HTML><html lang='en'><head><meta charset='utf-8'><title>AK Web Projector</title></head><body><h2 style='color:red'>Resource /%s not found </h2></body></html>",request->resource);
                    sprintf(responseBuffer,"HTTP/1.1 200 OK\nContent-Type:text/html\nContent-Length:%d\n\n",strlen(tmp));
    ...
    }// infinite loop related to accept method ends here
    /*if(successCode>0)
    {
